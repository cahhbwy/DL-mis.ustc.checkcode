...
def discriminator(image, label, training, reuse=None):
    with tf.variable_scope("discriminator", reuse=reuse):
        d_01 = tf.layers.conv2d(image, 80, 5, (1, 1), "valid", activation=tf.nn.relu, name="dis_01")  # 16x16x80
        d_02 = tf.layers.batch_normalization(d_01, momentum=0.95, epsilon=1e-4, training=training, name="dis_02")  # 16x16x80
        d_03 = tf.layers.max_pooling2d(d_02, 2, 2, "valid", name="dis_03")  # 8x8x80
        d_04 = tf.layers.conv2d(d_03, 200, 5, (1, 1), "valid", activation=tf.nn.relu, name="dis_04")  # 4x4x200
        d_05 = tf.layers.batch_normalization(d_04, momentum=0.95, epsilon=1e-4, training=training, name="dis_05")  # 4x4x200
        d_06 = tf.layers.max_pooling2d(d_05, 2, 2, "valid", name="dis_06")  # 2x2x200
...
def generator_0(rand, training, reuse=None):
    with tf.variable_scope("generator", reuse=reuse):
        ...
        g_02 = tf.layers.conv2d_transpose(g_01_bn, 40, 5, (2, 2), "same", activation=tf.nn.leaky_relu, name="gen_02")  # 10x10x40
        ...
        g_05 = tf.concat([g_02_bn, g_04_bn], axis=3, name="gen_05")  # 10x10x80
        ...
        g_09 = tf.concat([g_06_bn, g_08_bn], axis=3, name="gen_09")  # 20x20x40
...
def generator_1(rand, training, reuse=None):
    with tf.variable_scope("generator", reuse=reuse):
        g_00 = tf.layers.dense(rand, 2000, activation=tf.nn.leaky_relu, name="gen_00")  # 2000
        g_00_bn = tf.layers.batch_normalization(g_00, momentum=0.95, epsilon=1e-4, training=training, name="gen_00_bn")  # 2000
        g_01 = tf.reshape(g_00_bn, [-1, 5, 5, 80], name="gen_01")  # 5x5x80
        g_02 = tf.layers.conv2d_transpose(g_01, 40, 5, (2, 2), "same", activation=tf.nn.leaky_relu, name="gen_02")  # 10x10x40
        g_02_bn = tf.layers.batch_normalization(g_02, momentum=0.95, epsilon=1e-4, training=training, name="gen_02_bn")  # 10x10x40
        g_03 = tf.layers.conv2d_transpose(g_02_bn, 20, 5, (2, 2), "same", activation=tf.nn.leaky_relu, name="gen_03")  # 20x20x20
        g_03_bn = tf.layers.batch_normalization(g_03, momentum=0.95, epsilon=1e-4, training=training, name="gen_03_bn")  # 20x20x20
        g_04 = tf.layers.conv2d(g_03_bn, 10, 5, (1, 1), "same", activation=tf.nn.leaky_relu, name="gen_04")  # 20x20x10
...
def model(rand, batch_size):
    ...
    return training, real_image_uint8, fake_image_uint8, dis_loss, gen_loss
...
def train(start_step, restore):
    ...
    m_training, m_real_image, m_fake_image, m_dis_loss, m_gen_loss = model(m_rand, batch_size)
    ...
    dis_lr = tf.train.exponential_decay(learning_rate=0.00001, global_step=global_step, decay_steps=100, decay_rate=0.95)
