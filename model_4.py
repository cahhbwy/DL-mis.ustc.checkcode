...
def discriminator(batch_size, image, label_code, training=True, reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        d_01 = tf.layers.conv2d(d_00, 64, 5, (2, 2), "same", activation=tf.nn.leaky_relu, kernel_initializer=ki, name="dis_01")  # 10x10x64
        d_02 = tf.concat([d_01, tf.ones([batch_size, 10, 10, 32]) * tf.reshape(label_code, [batch_size, 1, 1, 32])], 3, name="dis_02")  # 10x10x96
        d_03 = tf.layers.conv2d(d_02, 128, 5, (2, 2), "same", activation=tf.nn.leaky_relu, kernel_initializer=ki, name="dis_03")  # 5x5x128
        d_04 = tf.layers.flatten(d_03, name="dis_04")  # 3200
        d_05 = tf.concat([d_04, label_code], 1, name="dis_05")  # 3232
        d_06 = tf.layers.dense(d_05, 1024, kernel_initializer=ki, name="dis_06")  # 1024
        d_07 = tf.layers.batch_normalization(d_06, momentum=0.9, epsilon=1e-3, training=training, name="dis_07")  # 1024
        d_08 = tf.nn.leaky_relu(d_07, name="dis_08")  # 1024
        d_09 = tf.concat([d_08, label_code], 1, name="dis_09")  # 1056
...
def generator(batch_size, rand_z, label_code, training=True, reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope("generator", reuse=reuse):
        ...
        g_01 = tf.layers.dense(g_00, 1024, kernel_initializer=ki, name="gen_01")  # 1024
        g_02 = tf.layers.batch_normalization(g_01, momentum=0.9, epsilon=1e-3, training=training, name="gen_02")  # 1024
        g_03 = tf.nn.relu(g_02, name="gen_03")  # 1024
        g_04 = tf.concat([g_03, label_code], 1, name="gen_04")  # 1056
        g_05 = tf.layers.dense(g_04, 3200, kernel_initializer=ki, name="gen_05")  # 3200
        g_06 = tf.layers.batch_normalization(g_05, momentum=0.9, epsilon=1e-3, training=training, name="gen_06")  # 3200
        g_07 = tf.reshape(g_06, [batch_size, 5, 5, 128], name="gen_07")  # 5x5x128
        g_08 = tf.concat([g_07, tf.ones([batch_size, 5, 5, 32]) * tf.reshape(label_code, [batch_size, 1, 1, 32])], 3, name="gen_08")  # 5x5x160
        g_09 = tf.layers.conv2d_transpose(g_08, 64, 5, (2, 2), "same", kernel_initializer=ki, name="gen_09")  # 10x10x64
        g_10 = tf.layers.batch_normalization(g_09, momentum=0.9, epsilon=1e-3, training=training, name="gen_10")  # 10x10x64
        g_11 = tf.nn.relu(g_10, name="gen_11")  # 10x10x64
        g_12 = tf.concat([g_11, tf.ones([batch_size, 10, 10, 32]) * tf.reshape(label_code, [batch_size, 1, 1, 32])], 3, name="gen_12")  # 10x10x96
        g_13 = tf.layers.conv2d_transpose(g_12, 1, 5, (2, 2), "same", activation=tf.nn.sigmoid, kernel_initializer=ki, name="gen_13")  # 20x20x1
        return g_13
...
def train(start_step, restore):
    ...
    with tf.control_dependencies(update_ops):
        dis_op = tf.train.AdamOptimizer(dis_lr, beta1=0.5).minimize(m_dis_loss, var_list=dis_vars)
        gen_op = tf.train.AdamOptimizer(gen_lr, beta1=0.5).minimize(m_gen_loss, var_list=gen_vars)
