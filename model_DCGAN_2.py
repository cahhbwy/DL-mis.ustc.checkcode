...
def train(start_step=0, restore=False):
    ...
    lr_dis = optimizers.schedules.ExponentialDecay(initial_learning_rate=0.00025, decay_steps=1000, decay_rate=0.999, staircase=False)
    lr_gen = optimizers.schedules.ExponentialDecay(initial_learning_rate=0.00045, decay_steps=1000, decay_rate=0.999, staircase=False)
    ...
    def train_step(image_real, repeat_dis, repeat_gen):
    ...
    for epoch in range(start_step, epochs):
        for image_batch in train_ds:
            train_step(image_batch, repeat_dis, repeat_gen)
